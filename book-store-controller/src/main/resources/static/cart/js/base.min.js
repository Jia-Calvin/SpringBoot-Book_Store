...
XIAOMI.namespace = function () {
    ...
    var L = XIAOMI.lang, OP = Object.prototype, ARRAY_TOSTRING = "[object Array]",
        FUNCTION_TOSTRING = "[object Function]", OBJECT_TOSTRING = "[object Object]", NOTHING = [], HTML_CHARS = {
            "&": "&amp;",
            "<": "&lt;",
            ">": "&gt;",
            '"': "&quot;",
...
;(function () {
    ...
    var _seedBytes = _rng(),
        ...
        _clockseq = 16383 & (_seedBytes[6] << 8 | _seedBytes[7]), _lastMSecs = 0, _lastNSecs = 0, uuid = v4;
...
;XIAOMI.namespace("app.analytics"), XIAOMI.app.analytics = function (element, options) {
    ...
    for (var _this = this, isURL = _this.options.url ? !0 : !1, trimAll = function (s) {
        ...
        $.each(links, function (k, v) {
            var paramString = isURL ? $(v).attr("href") : XIAOMI.lang.trim($(v).find("img").attr("alt") || $(v).text()),
                paramIndex = k, parmas_1 = _this.options.page,
                parmas_2 = parmas_1 + "_" + _this.options.position[i] + "_" + paramIndex,
                parmas_3 = parmas_2 + "_" + paramString, parmas = "'PC','" + parmas_3 + "'",
    ...
    var op = {obj: ".xmAddShopCart", callback: null, overCallback: null};
    $.extend(op, options || {}), $(op.obj).on("click", function () {
        var gid = $(this).attr("data-gid"), isDisabled = $(this).attr("data-disabled") || "false",
    ...
    init: function (options) {
        ...
        var _this = this, cbName = "cn" + _this.config.sku,
        ...
        $.ajax({
            ...
            success: function (data) {
                var jsonStr = data.status, token = jsonStr[_this.config.sku].hdurl,
    ...
    init: function () {
        ...
        var now = (new Date).getTime() / 1e3, startTime = new Date("2015/2/15 00:00:00").getTime() / 1e3,
    ...
    formatData: function (data) {
        ...
        if (0 === data.errorno && data.totalItem > 0) {
            var lens = data.items.length, goodsUrl = null, listHtml = "<ul>",
    ...
    constructor: XIAOMI.app.miniLogin, _proxyiframe: function () {
        ...
        function appendTobody(url) {
            var iframeURL = url ? url : _this.quickLoginUrl,
                ...
                boxWidth = $("#loginBox").attr("data-width"), boxHeight = $("#loginBox").attr("data-Height");
    ...
    function init() {
        var isCategoryStatic = "undefined" != typeof isCategoryToggled ? isCategoryToggled : !1, MI_SITES = {
            ...
            templateCategory = doT.template(templateHtml), oDate = new Date,
    ...
    function init(elm, opt) {
        ...
        var options, $listWrap, $list, $btnPrev, $btnNext, itemLength, itemWidth, itemHeight, itemPerSlide, countPage,
            $elm = $(elm), curIndex = 0;
        return 0 === $elm.length ? $elm : $elm.length > 1 ? ($elm.each(function () {
            init($(this), opt)
    ...
    speed: 200, inputObj: null, keyword: null, init: function () {
        ...
        var _this = this, $keywordList = $("#J_keywordList"), $items = $keywordList.find("li"),
            i = $keywordList.find(".current").index() || 0, lens = $items.length - 1;
        if (38 === keyCode) {
...
;!function () {
    ...
    doT.template = function (tmpl, c, def) {
        ...
        var needhtmlencode, indv, cse = c.append ? startend.append : startend.split, sid = 0,
